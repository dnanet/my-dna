...
<div class="single-job-listing <?php echo ! $listing_logo ? 'listing-no-logo' : '' ?>" id="c27-single-listing">
                    ...
                    <div class="profile-menu">
                        <ul>
                            ...
                            foreach ((array) $layout['menu_items'] as $key => $menu_item): $i++;
                                ...
                                ?><li>
                                    <a href="<?php echo esc_attr( '#'.$tab_id ) ?>" id="<?php echo esc_attr( 'listing_tab_'.$tab_id.'_toggle' ) ?>" data-section-id="<?php echo esc_attr( $tab_id ) ?>" class="listing-tab-toggle <?php echo esc_attr( "toggle-tab-type-{$menu_item['page']}" ) ?>" data-options="<?php echo c27()->encode_attr( (object) $tab_options ) ?>">
    ...
    <div class="tab-content listing-tabs">
        <?php foreach ((array) $layout['menu_items'] as $key => $menu_item): ?>
            <section class="profile-body listing-tab tab-hidden <?php echo esc_attr( "tab-type-{$menu_item['page']}" ) ?>" id="listing_tab_<?php echo esc_attr( $menu_item['slug'] ) ?>">
    ...
    <?php
$post_tag = str_replace(" ", "-", get_the_title());
    display_related_posts_on_listing($post_tag);
?>
    <?php
    /**
