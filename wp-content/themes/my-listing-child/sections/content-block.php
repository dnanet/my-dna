...
<div class="<?php echo esc_attr( $data['wrapper_class'] ) ?>" <?php echo $data['wrapper_id'] ? sprintf( 'id="%s"', $data['wrapper_id'] ) : '' ?>>
	<div class="element content-block <?php echo esc_attr( $data['escape_html'] ) ? 'plain-text-content' : 'wp-editor-content' ?>">
		...
		<div class="pf-body">
            <?php 
    //echo esc_attr( $data['wrapper_class']);
            if (strpos(esc_attr( $data['wrapper_class']),'iframe-src') !== false) {
                echo '<iframe src="'.wp_kses( nl2br( $data['content'] ), ['br' => []] ) .'"></iframe>';
            }
                elseif (strpos(esc_attr( $data['wrapper_class']), 'iframe-script') !== false) {
                    echo '<script src="'.wp_kses( nl2br( $data['content'] ), ['br' => []] ).'"></script>';
                }
                else {
    ?>
			<p>
