...
class Visitor {
	...
	public function __construct() {
		if ( ! isset( $_SESSION['mylisting']['guest'] ) ) {
			$_SESSION['mylisting']['guest'] = [];
		}
	}
	...
	public function get_location() {
		...
		if ( ! empty( $_SESSION['mylisting']['guest']['location'] ) ) {
			// mlog( 'Retrieved user location from session (IP geolocation).' );
			return $_SESSION['mylisting']['guest']['location'];
		}
		...
		// Store location in session, so we don't have to request it on every page visit.
		$_SESSION['mylisting']['guest']['location'] = $location;

		mlog()->warn( 'Retrieved user location from query. (IP geolocation).' );
		return $location;
	}
