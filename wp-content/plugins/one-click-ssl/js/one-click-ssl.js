(function($) {
	...
	$.fn.ocssl_step1 = function() {
		...
		$.ajax({
			url: ajaxurl + '?action=ocssl_check_ssl_support',
			type: "POST",
			dataType: "json"
	...
	$.fn.ocssl_step2 = function() {
		...
		$.ajax({
			url: ajaxurl + '?action=ocssl_enable_ssl',
			type: "POST"
		}).done(function (data, textStatus, jqXHR) {
			$loading.hide();
	...
	$(document).on('click', '.notice-one-click-ssl .notice-dismiss', function () {				
		...
		$.ajax(ajaxurl + '?action=ocssl_dismissed_notice', {
			type: 'POST',
			data: {
...
jQuery(document).ready(function() {	
	...
	if (typeof postboxes !== 'undefined') {
		postboxes.add_postbox_toggles("toplevel_page_one-click-ssl");
	}
});