	...
	class OCSSL {
		...
		function admin_menu() {
			...
			if (is_multisite() && is_network_admin()) {
				$this -> ocssl_menu = add_menu_page(__('One Click SSL', 'one-click-ssl'), __('One Click SSL', 'one-click-ssl'), 'activate_plugins', 'one-click-ssl', array($this, 'admin_network'), null, null);
			} else {
				$ocssl_toolsmenu = get_option('ocssl_toolsmenu');
				if (!empty($ocssl_toolsmenu)) {
					$this -> ocssl_menu = add_management_page(__('One Click SSL', 'one-click-ssl'), __('One Click SSL', 'one-click-ssl'), 'activate_plugins', 'one-click-ssl', array($this, 'admin'));
				} else {
					$this -> ocssl_menu = add_menu_page(__('One Click SSL', 'one-click-ssl'), __('One Click SSL', 'one-click-ssl'), 'activate_plugins', 'one-click-ssl', array($this, 'admin'), null, null);	
				}
			}
		...
		function admin_enqueue_scripts() {
			...
			if (!empty($_GET['page']) && ($_GET['page'] == 'one-click-ssl-setup' || $_GET['page'] == 'one-click-ssl')) {			
		...
		function make_request($url = null) {
			...
			if (function_exists('curl_init')) {								
				...
				curl_setopt($ch, CURLOPT_POSTFIELDS, array('ocssl_check' => true));
		...
		function ajax_scan() {
			...
			if (!empty($_POST['scanurl'])) {
				$scanurl = esc_html($_POST['scanurl']);
				$url .= ltrim($scanurl, '/');
			}
